...
fit.all.leaders <- survfit(Surv(time=tenuredem,event=rightcensdem) ~ 1,data=ddsurvival)
png("All leaders.png")
...
fit.by.democracy <- survfit(Surv(time=tenuredem,event=rightcensdem) ~ democracy,data=ddsurvival)
...
fit.by.region <- survfit(Surv(time=tenuredem,event=rightcensdem) ~ un_continent_name,data=subset(ddsurvival,democracy=="Non-democracy"))
...
fit.by.region <- survfit(Surv(time=tenuredem,event=rightcensdem) ~ un_continent_name,data=subset(ddsurvival,democracy=="Democracy"))
...
fit.by.coldwar <- survfit(Surv(time=tenuredem,event=rightcensdem) ~ coldwar,data=subset(ddcoldwar,democracy=="Non-democracy"))
fit.by.coldwar.dem <- survfit(Surv(time=tenuredem,event=rightcensdem) ~ coldwar,data=subset(ddcoldwar,democracy=="Democracy"))
